...
async fn create_device_password(
    ...
    // Fast path: Check device limit before expensive hashing to prevent DoS
    ...
    // Check device limit and insert atomically to prevent TOCTOU race conditions
    let device_opt = sqlx::query_as::<_, DevicePassword>(
        r#"
