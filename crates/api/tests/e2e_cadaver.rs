...
async fn run_cadaver_commands(url: &str, user: i64, pass: &str, commands: &[&str]) -> (bool, String, String) {
    ...
    #[cfg(unix)]
    {
        ...
        std::fs::set_permissions(&netrc_path, std::fs::Permissions::from_mode(0o600)).expect("Failed to set netrc permissions");
    }
    ...
    for cmd in commands {
        ...
        stdin.write_all(format!("{}\n", cmd).as_bytes()).await.expect("Failed to write command");
    }
    stdin.write_all(b"quit\n").await.expect("Failed to write quit");
    drop(stdin);

    let output = child.wait_with_output().await.expect("Failed to read cadaver output");
    (
...
async fn test_cadaver_full_flow(pool: PgPool) {
    ...
    let state = AppState {
        pool: pool.clone(),
        auth_cache: Cache::builder().time_to_live(Duration::from_secs(300)).build(),
    };
    ...
    tokio::spawn(async move {
        axum::serve(listener, app.into_make_service_with_connect_info::<std::net::SocketAddr>()).await.expect("Server failed");
    });
    ...
    let res = client.request(reqwest::Method::OPTIONS, &base_url)
    ...
    let (success, stdout, stderr) = tokio::time::timeout(Duration::from_secs(10), run_cadaver_commands(
        &base_url,
        telegram_id,
        &password,
        &["ls"]
    )).await.expect("Cadaver connection timed out") ;

    assert!(success, "cadaver ls failed!\nSTDOUT: {}\nSTDERR: {}", stdout, stderr);
    assert!(stdout.contains("Listing collection"), "Output missing 'Listing collection'");
    ...
    let (success, stdout, stderr) = tokio::time::timeout(Duration::from_secs(15), run_cadaver_commands(
        &base_url,
        telegram_id,
        &password,
        &[
    ...
    )).await.expect("Cadaver sequence timed out");

    let _ = std::fs::remove_file(ics_path);
    let _ = std::fs::remove_file("e2e-123-down.ics");

    assert!(success, "cadaver sequence failed!\nSTDOUT: {}\nSTDERR: {}", stdout, stderr);
    assert!(stdout.contains("Uploading"), "PUT missing! STDOUT: {}", stdout);
    assert!(stdout.contains("Downloading"), "GET missing! STDOUT: {}", stdout);
    assert!(stdout.contains("Deleting"), "DELETE missing! STDOUT: {}", stdout);
}
