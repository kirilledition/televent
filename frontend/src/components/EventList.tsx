...
export function EventList() {
    ...
    if (isLoading) {
        return <div className="p-4 text-center text-subtext0">Loading events...</div>;
    }

    if (error) {
        return <div className="p-4 text-center text-red">Error loading events.</div>;
    }

    if (!events || events.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center p-8 text-overlay2">
...
function EventCard({ event, onDelete }: { event: EventResponse; onDelete: () => void }) {
    const start = new Date(event.start);
    const end = new Date(event.end);
    const router = useRouter();

    return (
        <div className="bg-surface0 border-surface1 flex flex-col gap-2 rounded-xl border p-4 shadow-sm transition-all hover:bg-surface1/50 active:scale-[0.99]">
            <div className="flex items-start justify-between gap-2">
                <div>
                    <h3 className="text-text font-semibold line-clamp-1">{event.summary}</h3>
