...
export function CreateEvent({ onClose, onCreate, initialEvent }: CreateEventProps) {
  ...
  const [date, setDate] = useState(initialEvent?.date || now.toISOString().split('T')[0]);
  ...
  return (
    <div className="w-full sm:max-w-md mx-auto min-h-screen" style={{ backgroundColor: 'var(--ctp-base)' }}>
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 sticky top-0 z-10" style={{ backgroundColor: 'var(--ctp-base)', borderBottom: '1px solid var(--ctp-surface0)' }}>
          <button
            type="button"
            onClick={onClose}
